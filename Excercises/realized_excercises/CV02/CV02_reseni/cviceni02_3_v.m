...
t = 0:0.01:60;                          % Generovani casoveho vektoru
u = ones(size(t));                      % Generovani vstupniho vektoru - skok v case 0
y = cviceni02_3(u,t);                   % Odezva systemu na vstupni data
...
plot(t, d*t+yi-d*ti,'--r',t(2:end),diff(yn)./(t(2)-t(1)),'c')   %Tyrkysove - zobrazeny derivace
plot(t,(u(1:end)-1),'--k')
...
tau2=0.2639;                            % dosazeni tabulkove hodnoty tau2 z hodnoty y2
T1 = T12 / (1+tau2);
T2 = T12 - T1;      
s = tf('s');
